"""Detector checking for product comparison intent."""

import re
import time

from src.ai_agents.enums.intent import IntentEnum
from src.ai_agents.intent.constants import COMPARISON_KEYWORDS
from src.ai_agents.intent.result import DetectorResult


class ComparisonDetector:
    """Detector identifying when users want to compare laptops or mobile phones."""
    ...
    def detect(self, query: str) -> DetectorResult:
        ...
        # Scan for word boundary matches on comparison keywords
        ...
        # Calculate raw confidence: base score of 0.80 if matched, plus keyword density
        confidence = 0.0
        if matched:
